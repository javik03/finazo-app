...
function mortgageItinStateTopic(state: typeof PRIORITY_STATES[number]): UsContentTopic {
  return {
    slug: `hipoteca-itin-${state.slug}-2026`,
    category: "prestamos",
    imageQuery: `${state.nameEn} suburban home hispanic family`,
    preferredAuthor: "javier-keough",
    prompt: `Eres un experto en hipotecas ITIN en ${state.nameEs}. Escribe una guía en español de 1200-1400 palabras.

Keyword principal: "hipoteca ITIN ${state.nameEs}" (con "sin Social Security" como keyword secundaria)
Título H1: "Hipoteca con ITIN en ${state.nameEs}: wholesalers y tasas (2026)"

Contexto: ${state.insuranceNote}.

Estructura:
## Introducción (keyword primeras 100 palabras)
## Sí se puede comprar casa con ITIN en ${state.nameEs}
## Wholesalers ITIN activos en ${state.nameEs}
(Tabla: Wholesaler | Tasa típica sobre conv. | Down payment mín. | LTV máx.)
## Down payment típico (10-25%)
## Documentos que necesitas con ITIN
## Programas estatales para first-time buyer en ${state.nameEs}
## Hogares — pre-calificación por WhatsApp en 24h
(divulgación)
## Errores que rechazan tu hipoteca con ITIN
## Preguntas frecuentes
## Conclusión${US_SEO_SUFFIX}`,
  };
}
...
function cityMortgageTopic(city: typeof PRIORITY_CITIES[number]): UsContentTopic {
  return {
    slug: `comprar-casa-itin-${city.slug}-2026`,
    category: "prestamos",
    imageQuery: `${city.nameEs} skyline residential homes`,
    preferredAuthor: "javier-keough",
    prompt: `Eres un experto en compra de vivienda ITIN en ${city.nameEs}, ${city.state}. Escribe una guía en español de 1200-1400 palabras.

Keyword principal: "comprar casa con ITIN en ${city.nameEs}"
Título H1: "Cómo comprar casa con ITIN en ${city.nameEs} (${city.state}): guía 2026"

Contexto del mercado: ${city.context}. Precios medianos: ${city.homePrice}.

Estructura:
## Introducción (keyword primeras 100 palabras; precio mediano del mercado local)
## Realidad del mercado: ${city.nameEs} 2026
(Tabla: Zona | Precio mediano | Tipo de inventario | Tiempo en mercado)
## Wholesalers ITIN activos en ${city.nameEs}
...
## Hogares — broker non-QM ITIN
(divulgación)
## Errores comunes en ${city.nameEs}
## Preguntas frecuentes
## Conclusión${US_SEO_SUFFIX}`,
  };
}
...
export function getAllProgrammaticTopics(): UsContentTopic[] {
  return [
    // Carrier comparisons (high commercial intent)
    ...CARRIER_COMPARISONS.map(comparisonTopic),
    // Lender reviews (high commercial intent)
    ...LENDER_REVIEWS.map(lenderReviewTopic),
    // State auto insurance (geo-targeted, broad volume)
    ...PRIORITY_STATES.map(autoInsuranceStateTopic),
    // State ITIN mortgage (commercial intent, Hogares funnel)
    ...PRIORITY_STATES.map(mortgageItinStateTopic),
    // City ITIN mortgage (long-tail, Hogares funnel)
    ...PRIORITY_CITIES.map(cityMortgageTopic),
    // Remittance corridors
    ...REMITTANCE_DESTINATIONS.map(remittanceCorridorTopic),
    // Glossary (informational, builds topical authority)
